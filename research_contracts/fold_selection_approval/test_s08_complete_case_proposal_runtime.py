...
from research_contracts.fold_selection_approval import (  # noqa: E402
    s08_complete_case_proposal_runtime as runtime,
)
...
def fixture():
    ...
    for ticker_index, ticker in enumerate(tickers):
        first = 1 if ticker == "FISV" else 59 if ticker == "SNDK" else 0
        for day_index, session in enumerate(dates[first:], first):
...
class SelectOnlyProposalRuntimeTests(unittest.TestCase):
    ...
    def assemble(self, client=None, bundle=None, pins=None):
        with patch.dict(runtime.EXPECTED_RAW_SHA256, self.hashes, clear=True):
            return runtime.assemble_v6_proposal(
    ...
    def test_exact_audit_only_assembly_and_zero_output_boundary(self):
        ...
        self.assertEqual(result.panel_shape, (472, 416))
        self.assertEqual(result.excluded_tickers, (("FISV", 416), ("SNDK", 358)))
        lineage = json.loads(result.proposal.artifacts["lineage.json"])
        self.assertEqual(result.presence_mask_sha256,
                         lineage["presence_mask_sha256"])
    ...
    def test_artifact_tamper_fails_before_database(self):
        ...
        bad = replace(self.bundle, selector_v8=self.bundle.selector_v8 + b"tamper")
        before = len(client.calls)
        with patch.dict(runtime.EXPECTED_RAW_SHA256, self.hashes, clear=True):
            with self.assertRaisesRegex(runtime.SelectOnlyAssemblyError, "artifact bytes differ"):
                runtime.assemble_v6_proposal(
    ...
    def test_stale_s07_readback_remains_unsigned(self):
        with patch.dict(runtime.EXPECTED_RAW_SHA256, self.hashes, clear=True):
            result = runtime.assemble_v6_proposal(
    ...
    def test_canonical_artifact_hashes_match_b99_contract_release(self):
