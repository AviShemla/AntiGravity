#!/usr/bin/env python3
"""Root-only, write-once wrapper for the inert S08 SELECT-only proposal runtime."""
...
from .s08_select_only_proposal_runtime import (
    AuditPins, SelectOnlyAssemblyError, assemble_v5_proposal,
...
CONTRACT_ID = "codex-oracle-s08-select-only-proposal-runner-v1"
...
def _proposal_summary(proposal) -> dict[str, object]:
    return {
        ...
        "approval_record_sha256": proposal.approval_record_sha256,
    ...
    *, client: object, repository_root: Path, s07_directory: Path,
    ...
    payload = {
        ...
        "runtime_source_sha256": hashlib.sha256(
            Path(__file__).with_name("s08_select_only_proposal_runtime.py").read_bytes()
        ).hexdigest(),
