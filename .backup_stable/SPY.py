...
def get_active_tickers(folder_path):
    ...
    try:
        import glob
        import pandas as pd
        ledger_files = glob.glob(os.path.join(folder_path, 'Capital_Ledger_*.csv'))
        # Exclude ETF ledger since SPY.py is for single stocks
        ledger_files = [f for f in ledger_files if 'ETF' not in os.path.basename(f)]
        legacy_holdings = set()
        ...
        for lf in ledger_files:
            try:
                df = pd.read_csv(lf)
                if not df.empty and 'Holdings_JSON' in df.columns:
                    holdings_str = df.iloc[-1]['Holdings_JSON']
                    holdings = json.loads(holdings_str)
                    for t in holdings.keys():
                        if t not in all_vip_tickers:
