...
OUTPUT_EXCEL = os.path.join(BASE_DIR, 'MultiPersona_Broker_30Day_Trial.xlsx')
...
def process_ledger(persona_name):
    ...
    path = os.path.join(BASE_DIR, f'Capital_Ledger_{file_persona}.csv')
...
def generate_excel():
    ...
    for p in PERSONAS:
        ...
        path = os.path.join(BASE_DIR, f'Capital_Ledger_{file_p}.csv')
    ...
    # Forward-fill any NaN values caused by pipeline desyncs to keep line charts connected
    ...
    for col_num, col_name in enumerate(merged_df.drop(columns=['Week']).columns):
        if 'Equity' in col_name or 'Cash' in col_name or 'Profit_$' in col_name:
            ...
            worksheet1.set_column(col_num, col_num, 60, wrap_fmt)
        else:
    ...
    colors = ['#2ECC40', '#1E90FF', '#FF851B'] # Vibrant Green, Bright Blue, Bright Orange
    
    for idx, p in enumerate(PERSONAS):
        col_idx = cols.index(f'{p}_Total_Equity')
        
        # Add smooth glow/shadow effect to lines if supported, or just sleek lines
        chart.add_series({
            'name':       p,
            'categories': ['Daily Tracking', 1, 0, max_row, 0],
            'values':     ['Daily Tracking', 1, col_idx, max_row, col_idx],
            'line':       {'color': colors[idx], 'width': 2.5},
            'marker':     {'type': 'circle', 'size': 5, 'border': {'color': colors[idx]}, 'fill': {'color': '#111111'}}
        })
        
    chart.set_title({
        'name': '30-Day Multi-Persona Equity Race',
        'name_font': {'color': 'white', 'size': 14, 'bold': True}
    })
    ...
    chart.set_y_axis({
        ...
        'num_font': {'color': 'white'},
        'num_format': '$#,##0',
        'major_gridlines': {'visible': True, 'line': {'color': '#2a2a2a', 'dash_type': 'dash'}},
        'line': {'color': '#444444'}
    })
    ...
    worksheet1.insert_chart('T2', chart)
