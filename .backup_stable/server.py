...
def get_dropdown_options(persona: str = "BallsForBrains", mode: str = "Single"):
    ...
    ledger_path = os.path.join(BASE_DIR, f'Capital_Ledger_{persona}.csv' if mode == 'Single' else f'ETF_Capital_Ledger_{persona}.csv')
...
def get_bayesian_data(ticker: str, persona: str = "BallsForBrains", mode: str = "Single"):
    ...
    try:
        ...
        for p in ["Conservative", "Neutral", "BallsForBrains"]:
            lp = os.path.join(BASE_DIR, f'Capital_Ledger_{p}.csv' if mode == 'Single' else f'ETF_Capital_Ledger_{p}.csv')
            if os.path.exists(lp):
                df_p = pd.read_csv(lp)
...
def get_olympic_data():
    cap_path = os.path.join(BASE_DIR, 'financial_data', 'Backtest_Ledger_EL_CAP.csv')
    vol_path = os.path.join(BASE_DIR, 'financial_data', 'Backtest_Ledger_EL_VOLTI.csv')
    champ_path = os.path.join(BASE_DIR, 'financial_data', 'Backtest_Ledger_CHAMPION.csv')
    
    if not (os.path.exists(cap_path) and os.path.exists(vol_path) and os.path.exists(champ_path)):
        raise HTTPException(status_code=404, detail="Olympic backtest ledgers not found")
...
def get_autopsy_data():
    stock_ledger = os.path.join(BASE_DIR, 'Capital_Ledger_BallsForBrains.csv')
