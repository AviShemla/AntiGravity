...
# Risk-Adjusted Returns (Return / STDEV_20d)
# This amplifies the signals of STABLE stocks and shrinks the noise of VOLATILE stocks.
# We add a tiny epsilon to avoid division by zero just in case.
risk_adj_returns = returns_df / (stddev_pivot + 1e-8)

# MA3 of the Risk-Adjusted Returns
ma3_risk_adj_returns = risk_adj_returns.rolling(window=3).mean()
...
if __name__ == '__main__':
    ...
    for l in [1, 2, 3]:
        res = run_pymc_experiment(target_returns, risk_adj_returns, l, "Risk-Adjusted Daily")
        results.append(res)
        
    for l in [1, 2, 3]:
        res = run_pymc_experiment(target_returns, ma3_risk_adj_returns, l, "Risk-Adjusted MA3")
        results.append(res)
        
    summary_df = pd.DataFrame(results)
    print("\n\n=== Final PyMC Risk-Adjusted Workflow Summary ===")
    print(summary_df.to_string(index=False))
    
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PyMC_Risk_Adjusted_Results.csv')
