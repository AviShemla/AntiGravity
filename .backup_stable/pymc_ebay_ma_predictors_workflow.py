...
# Macro predictors (same for all tickers on a given date)
...
print("Applying 3-day Moving Average to predictors...")
all_predictors_df = all_predictors_df.rolling(window=3).mean()
...
if __name__ == '__main__':
    ...
    # Target is EBAY raw returns (NOT moving average)
    target_returns = return_pivot[champion_ticker].rename(f'{champion_ticker}_Target')
    
    for l in [1, 2, 3]:
        res = run_pymc_experiment(target_returns, all_predictors_df, l, "EBAY MA3 Predictors")
        results.append(res)
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PyMC_EBAY_MA3_Predictors_Results.csv')
