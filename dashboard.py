...
def get_engine_version(persona="Dynamic"):
    ...
    if df.empty or 'Engine_Version' not in df.columns:
        return "V1.0 - Pure PyMC Bayesian"
    return df.iloc[-1]['Engine_Version']
    ...
    for _, row in ledger.iterrows():
        ...
        for asset, pnl in pnl_dict.items():
            ...
            if pnl_val < -1.0:  # Meaningful loss > $1
                ...
                blacklist_status = blacklisted_set.get(asset, "OK") if isinstance(blacklisted_set, dict) else ("Blacklisted (≥3 Strikes)" if asset in blacklisted_set else "OK")
...
try:
    img_base64 = get_base64_of_bin_file(os.path.join(BASE_DIR.replace("financial_data", ""), "frontend", "oracle_logo_unleash.png"))
    st.markdown(
        f'''
...
# Fetch dynamic engine version from the SSOT
active_version = get_engine_version()
st.markdown(f"Read-Only Live Web Dashboard | 🧠 **Active AI Engine:** `{active_version}`")

tab1, tab2, tab3, tab4 = st.tabs([":material/business_center: Single Stocks", "🪐 ETF Markets", "🏆 Bayesian Olympics", "☠️ Trade Autopsy"])
...
def get_task_status(task_name):
    try:
        out = subprocess.check_output(f'schtasks /query /tn "{task_name}" /fo csv /nh', shell=True, stderr=subprocess.DEVNULL).decode()
        parts = out.strip().split('","')
        if len(parts) >= 3:
            next_run = parts[2].replace('"', '')
            if next_run == "N/A":
                return "Standby"
            return next_run.split(" ")[1] if " " in next_run else next_run
    except:
        return "Not Scheduled"
    return "Not Scheduled"

t_daily = get_task_status("AntiGravity_Daily_Pipeline")
t_weekend = get_task_status("AntiGravity_Weekend_Backtest")
...
<div class="cron-widget">
    <div class="cron-header">
        <span style="font-size: 18px;">⚙️</span> System Cron Jobs
    </div>
    <div class="cron-item">
        <span>🤖 Daily Pipeline</span>
        <span class="cron-status">{t_daily}</span>
    </div>
    <div class="cron-item">
        <span>🧬 Weekend Backtest</span>
        <span class="cron-status">{t_weekend}</span>
    </div>
    <div class="cron-item">
        <span>🌐 Live Dashboard</span>
        <span class="cron-status">Active</span>
    </div>
</div>
""", unsafe_allow_html=True)
