...
import pymc as pm
import pytensor
pytensor.config.intX = "int32"
import engine_config

# 1. ENFORCE GLOBAL BAYESIAN ENGINE CONFIGURATION FIRST
...
def evaluate_ticker(ticker, lags_dict, returns_df, shifted_preds, start_date, next_biz_day, fundamental_score=0.0):
    ...
    y_mag_train = y_mag_train.astype(float)
...
if __name__ == '__main__':
    ...
    for i, ticker in enumerate(tickers_list):
        ...
        with open(state_file, 'w') as sf:
            import json
            json.dump({"total_tickers": len(tickers_list), "processed_models": i}, sf)
        ...
        if ticker not in returns_df.columns:
            print(f"\n=> [ZOMBIE FAILSAFE] Skipping {ticker}: Missing from historical data.")
            continue
            
        corr_series = returns_df.corrwith(returns_df[ticker]).abs().sort_values(ascending=False)
        parent_ticker = corr_series.index[1] if len(corr_series) > 1 else ticker
        
        lags_dict = {}
        for d in range(depth, 0, -1):
            lags_dict[d] = parent_ticker
