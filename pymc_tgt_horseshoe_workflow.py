...
def run_pymc_horseshoe(target_series, predictors_df, lag, exp_name):
    ...
    # Pre-filter to Top 15 to give Horseshoe a pool to work with without stalling Python backend
    corrs = X_pool_train.corrwith(pd.Series(y_train, index=X_pool_train.index))
    top_features = corrs.abs().sort_values(ascending=False).head(15).index.tolist()
    print(f"Top 15 predictors pre-selected for Horseshoe.")
    
    X_train = X_pool_train[top_features].values
    X_test = X_pool_test[top_features].values
    
    with pm.Model() as model:
        # Horseshoe Prior implementation
        tau = pm.HalfStudentT("tau", nu=1, sigma=0.1) # Global Shrinkage
        lam = pm.HalfStudentT("lam", nu=1, sigma=1, shape=len(top_features)) # Local Shrinkage
        beta = pm.Normal("beta", mu=0, sigma=tau * lam, shape=len(top_features))
        ...
        # We increase target_accept to 0.95 to help sampler handle the Horseshoe funnel
        trace = pm.sample(draws=500, tune=500, chains=2, target_accept=0.95, random_seed=42, progressbar=False)
    ...
    # Find which features were NOT shrunk heavily (using a small threshold)
    active_features = []
    for i, b in enumerate(beta_post):
        if abs(b) > 1e-4:  # Threshold to consider a feature "active"
            active_features.append(top_features[i])
            
    if len(active_features) > 5:
        active_str = ", ".join(active_features[:5]) + f" (+{len(active_features)-5} more)"
    elif len(active_features) > 0:
        active_str = ", ".join(active_features)
    else:
        active_str = "All shrunk to exactly zero"
        
    print(f"Active features selected by Horseshoe: {len(active_features)}")
    ...
    return {'Experiment': exp_name, 'Lag': lag, 'Test R2': r2_test, 'Predictors': active_str}
...
if __name__ == '__main__':
    ...
    for l in [1, 2, 3]:
        res = run_pymc_horseshoe(target_returns, all_predictors_df, l, "TGT Horseshoe")
        results.append(res)
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PyMC_TGT_Horseshoe_Results.csv')
