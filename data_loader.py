...
def load_predictors(ticker=None):
    ...
    # ---- Macro features ----
    macro_cols = [vix_col, 'Market_Fear_Num']
    if 'TNX_Trend_5d' in df.columns:
        macro_cols.append('TNX_Trend_5d')
...
def extract_train_test_split(data_with_future, next_biz_day, feat_cols):
    """
    Standardized array extraction logic that perfectly handles both 
    historical backtests and forward-looking live predictions without duplicates.
    """
    ...
    # Standardized logic to prevent duplicates in backtests vs live predictions
    ...
    return X_train, y_train, y_mag_train, X_test, y_test, raw_return_test, split_idx, test_data, returns_full
