...
print(f"Loading data for {ticker} (Expanded Period: 2024-06-01 to present)...")
...
print(f"Training Start: {start_date.date()}")
print(f"Causal Chain: {yyy} -> {zzz} -> {xxx}")

target_t = returns_df[ticker].rename('Target_t')

# 1. Technical Features (re-evaluate correlations over this new 2-year period)
...
print(f"Top 3 Technicals (2024-2026): {top_3_tech}")
...
# 3. Train/Test Split (Keep the same 30-day OOS period so it's an apples-to-apples comparison)
...
print(f"\n=== RESULTS FOR {ticker} (TRAINED ON 2 YEARS DATA) ===")
...
# Compare against the 12-month result
print("\n--- PREVIOUS 12-MONTH RESULT ---")
print("Overall OOS Accuracy: 70.0%")
print("High-Confidence (>65%) Acc: 76.5% (17 Trades)")
