...
from oracle_production_adapter_authorization import authorize_then_create_transport
...
class _AuthorizedOperation:
    """Runtime identity produced only after the external envelope is verified."""
...
_AUTHORIZED_CONSTRUCTION = object()
...
class InjectedTursoImmediateTransactionRunner:
    """Atomic runner whose transport cannot exist before envelope validation."""

    def __init__(
        self,
        transport: InjectedAtomicPipelineTransport,
        authorization: _AuthorizedOperation,
        *,
        _construction_token: object,
    ):
        if _construction_token is not _AUTHORIZED_CONSTRUCTION:
            raise LineageError(
                "Oracle research runner must be created through the authorization envelope."
            )
        if transport is None:
            raise LineageError("Oracle research atomic transport is required.")
        if not isinstance(authorization, _AuthorizedOperation):
            raise LineageError("Validated Oracle operation identity is required.")
        self.transport = transport
        self.authorization = authorization

    @classmethod
    def from_authorization_envelope(
        cls,
        transport_factory: Callable[[], InjectedAtomicPipelineTransport],
        *,
        ...
        """Validate immutable trust leaves before constructing any transport."""

        authorization_snapshot = dict(authorization)
        dataset_id = authorization_snapshot.get("authorized_dataset_version_id")
        event_id = authorization_snapshot.get("authorized_freeze_event_id")
        if not isinstance(dataset_id, str) or not isinstance(event_id, str):
            raise LineageError("Oracle operation identities must exist before transport.")
        transport, trusted_envelope_sha256 = authorize_then_create_transport(
            transport_factory,
            envelope=envelope,
        ...
        return cls(
            transport,
            _AuthorizedOperation(
                envelope_sha256=trusted_envelope_sha256,
                operation_id=operation_id,
                dataset_version_id=dataset_id,
                freeze_event_id=event_id,
            ),
            _construction_token=_AUTHORIZED_CONSTRUCTION,
        )

    def _operation_contract(self, operation_id: str) -> tuple[str, ...]:
        if operation_id != self.authorization.operation_id:
            raise LineageError("Oracle research operation is outside the approved identity.")
        dataset_id = self.authorization.dataset_version_id
        event_id = self.authorization.freeze_event_id
