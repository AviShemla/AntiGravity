...
def get_ledger_stats(persona_name):
    ...
    try:
        df = database_manager.get_ledger(persona_name)
        ...
        if len(df) < 2:
            return {
                "date": last_row['Date'],
                "equity": float(last_row['Total_Equity']),
                "pnl": 0.0,
                "pct": 0.0
            }
        ...
        print(f"Error reading DB for {persona_name}: {e}")
        return None
...
def send_executive_brief():
    ...
    for p in personas:
        s_stats = get_ledger_stats(p)
        e_stats = get_ledger_stats(f"ETF_{p}")
    ...
    <div style="background-color: #f8f9fa; padding: 20px; border-radius: 10px; border: 1px solid #ddd;">
        ...
        <div style="background-color: white; padding: 20px; border-radius: 8px; text-align: center; border: 2px solid {color};">
            <h3 style="margin: 0; color: #555;">Total Global Daily PnL</h3>
            <h1 style="margin: 10px 0; color: {color};">{sign}${total_pnl:,.2f}</h1>
            
            <table align="center" style="margin: 0 auto 15px auto; border-collapse: collapse;">
                <tr>
                    <td align="center" style="background-color: #e8f4fd; padding: 10px 25px; border-radius: 8px; border: 1px solid #cce5ff;">
                        <span style="font-size: 11px; color: #555; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">Single Stocks</span><br>
                    ...
                    <td width="20"></td>
                    <td align="center" style="background-color: #f8f0fc; padding: 10px 25px; border-radius: 8px; border: 1px solid #e8d0fa;">
                        <span style="font-size: 11px; color: #555; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">ETF Markets</span><br>
