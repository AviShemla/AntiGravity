...
1. <span style="color: #FF851B; font-weight: bold;">[PENDING]</span> **Cloud Migration & External DB:** Plan and execute the migration of the entire master pipeline, ledgers, and dashboard to a secure external cloud database (AWS/Oracle) to support massive datasets.
2. <span style="color: #FF851B; font-weight: bold;">[PENDING]</span> **Non-Linear Deep Learning:** Once the external DB is live to handle the data load, explore LSTMs or Transformer models capable of finding complex, non-linear sequence patterns for hidden alpha.
