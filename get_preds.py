...
try:
    s = pd.read_excel(r'C:\Users\AviShemla\AntiGravity\financial_data\Top5_Bayesian_Scorecard_Formatted.xlsx', sheet_name=None)
    print("\n[STOCKS]")
    for k, v in s.items():
...
try:
    e = pd.read_excel(r'C:\Users\AviShemla\AntiGravity\financial_data\All_ETFs_Scorecard.xlsx', sheet_name=None)
    print("\n[ETFs]")
    for k, v in e.items():
        if k != 'Sheet1' and not v.empty:
            ret = v.iloc[-1].get("Next Day Expected Return", 0) * 100
