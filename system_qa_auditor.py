...
def phase_e_pre_market_validation():
    ...
    try:
        ...
        for p in expected_personas:
            # Check pending_orders first
            p_data = execute_query(f"SELECT persona FROM pending_orders WHERE date = '{expected_target}' AND persona = '{p}'")
            if p_data.empty:
                # If missing from pending_orders, it might have been consumed intraday!
                # Check capital_ledgers
                ledger_data = execute_query(f"SELECT persona FROM capital_ledgers WHERE date = '{expected_target}' AND persona = '{p}'")
                if ledger_data.empty:
                    missing_or_stale.append(p)
                
        if missing_or_stale:
            print(f" !!! FAIL: Missing Pending Orders for Target Date {expected_target} for personas: {missing_or_stale}")
            return False
            
        print(f" -> PASS: All {len(expected_personas)} Personas successfully verified in Capital Ledgers/Pending Orders for target date {expected_target}.")
        return True
        df = execute_query(query)
        if df.empty:
...
def main():
    ...
    if all_passed:
        ...
        import sys; sys.stdout.flush(); os._exit(1)
