...
if __name__ == '__main__':
    ...
    def run_simulation(test_name, tickers_list, start_cash=10000.0):
        ...
        for i, date in enumerate(sim_dates):
            ...
            # 2. Spawn PyMC Workers in parallel for `date_str`
            ...
            if os.path.exists(out_json):
                ...
                print(f"  Spawning Parallel Bayesian Subprocesses for {len(tickers_list)} tickers (Memory Protection Active)...")
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
                    futures = {executor.submit(run_single_ticker, t): t for t in tickers_list}
                    for i, future in enumerate(concurrent.futures.as_completed(futures)):
                        t_out = future.result()
                        if t_out and os.path.exists(t_out):
                            try:
                                with open(t_out, 'r') as f:
                                    preds_merged.update(json.load(f))
                            except: pass
                            os.remove(t_out)
                        if (i+1) % 10 == 0:
                            print(f"    Completed {i+1}/{len(tickers_list)} tickers...")
                            
                with open(out_json, 'w') as f:
                    json.dump(preds_merged, f)
