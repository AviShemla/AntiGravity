        ...
        if os.path.exists(fund_path) and whales_dict:
            ...
            for whale, weight in whales_dict.items():
                if whale in fund_df.index:
                    # handle possible duplicate indices if CSV is messy
            ...
            if total_weight > 0:
                etf_agg_score = weighted_sum / total_weight
                print(f"  [ETF PRIOR] Calculated Aggregate Fundamental Score for {target_etf}: {etf_agg_score:.3f}")
    except Exception as e:
        print(f"  [ETF PRIOR] Failed to calculate fundamental prior: {e}")
    # -----------------------------------
